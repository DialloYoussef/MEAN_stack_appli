...
export class EtudiantComponent {
  ...
  etudiants: any;
  myForm: FormGroup;
  // Variable pour indiquer si le formulaire est en mode création ou modification
  isCreationMode: boolean = true;
  // Variable pour stocker l'ID de l'étudiant à modifier
    ...
    if (this.myForm.valid) {
      if (this.isCreationMode) {
        ...
        if (this.etudiantId) {
          ...
          this.service
            .modifierEtudiant(this.etudiantId, this.myForm.value)
            .subscribe({
              next: (response) => {
                this.resetData();
              },
